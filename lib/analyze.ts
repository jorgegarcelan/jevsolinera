// El análisis completo para un punto y un combustible. Lo usan la API (cada visita)
// y la tarea diaria (que registra predicciones aunque nadie entre).

import { decide, type TankId, type Trend } from "./decide";
import { openState } from "./hours";
import { FUELS, distanceKm, getCurrentStations, getHistory, madridDate, median, type FuelId, type Prices } from "./minetur";
import { getBrent, getHeadlines } from "./signals";
import type { Prediction } from "./track";
import type { Analysis, StationResult } from "./types";
...
export async function analyze({ lat, lon, fuel, tank, liters }: AnalyzeInput): Promise<{
  analysis: Omit<Analysis, "track">;
  ...
  try {
    ...
    throw new AnalyzeError("El servicio de precios del Ministerio no responde. Prueba en un minuto.", 502);
  }
  ...
  if (!nearby.length) throw new AnalyzeError(`No hay gasolineras con ${FUELS[fuel].label} a menos de 50 km.`, 404);
  ...
  // Primero las abiertas (o de horario desconocido), por lo que cuesta ir y llenar;
  // las cerradas al final: no tiene sentido recomendar una que no te va a atender.
  const week = hist.get(7);
  const now = new Date();
  const stations: StationResult[] = nearby
    .map(({ s, d }) => {
      ...
      return {
        ...
        ...openState(s.hours, s.provinceId, now),
      };
    })
    .sort((a, b) => Number(a.open === false) - Number(b.open === false) || a.totalCost - b.totalCost);
